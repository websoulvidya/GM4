...
function Dailymatch() {
  ...
  useEffect(() => {
    axios.get(`https://gm4-server.herokuapp.com/api/user/read/all/daily/matches/${localStorage.getItem('id')}`, { headers: { "Authorization": `Bearer ${token}` } }).then((response) => {
        ...
        <div className='dailymatch-card-list'>
          <div class="uk-child-width-1-4@l uk-child-width-1-3@m uk-grid uk-text-center main-card" uk-grid>
            ...
            const maximumVal = 50;
                ...
                let matchDateDMY = `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;
              return (
                <div className='dailymatch-carditems'>
                  <div class="dailymatch-card">
                    ...
                    <div className='dailymatch-date'>
                      ...
                      <span classname="dailymatch-matchdate">{matchDateDMY}</span>
                    </div>
                    <div className='dailymatch-progress-bar'>
                      <progress value={val.numberOfRegs} max="100" className='dailyprogress'>

                      </progress>

                      <p className='progressbar-left-text'>{val.numberOfRegs} Joined</p>
                      <p className='progressbar-right-text'>{maximumVal-(val.numberOfRegs)} Spot left</p>
                    </div>
                    <div className='dailymatch-reg-section'>
                      ...
                      <Link to={`/dailymatch/dailymatchreg/${val._id}`}>Reg Now</Link>
                    </div>
                  </div>
                </div>
